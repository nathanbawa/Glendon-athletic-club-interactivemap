...
// ─────────────────────────────────────────
// GOOGLE EARTH STYLE — LEFT/RIGHT SWIPE ONLY
// ─────────────────────────────────────────
const DEG = Math.PI / 180;

// Sensitivity: degrees of theta rotation per pixel of horizontal drag
// Lower = slower/heavier feel, higher = faster/lighter
...
// Velocity tracking for momentum/glide after finger lifts
let velocity = 0;
let lastDeltaX = 0;
let momentumId = null;
...
const applyMomentum = () => {
  ...
  // Only change theta, phi and radius are locked by min/max-camera-orbit
  mv.cameraOrbit = `${o.theta - velocity * DEG}rad ${o.phi}rad ${o.radius}m`;
  velocity *= 0.92; // friction — higher = more glide, lower = stops faster
...
mv.addEventListener('touchmove', (e) => {
  // Ignore multi-touch — no zoom, no tilt
  if (e.touches.length !== 1) return;

  const t = e.touches[0];
  const dx = t.clientX - lastTouchX;
  const totalMove = Math.abs(t.clientX - touchStartX);

  if (totalMove > TAP_MAX_MOVE_PX) touchDidMove = true;

  if (touchDidMove) {
    const o = mv.getCameraOrbit();
    // Subtract dx so dragging right rotates the view right (natural Google Maps feel)
    const newTheta = o.theta - (dx * ROTATE_SENSITIVITY * DEG);
    mv.cameraOrbit = `${newTheta}rad ${o.phi}rad ${o.radius}m`;
    lastDeltaX = dx;
  }
...
mv.addEventListener('touchend', (e) => {
  ...
  const wasTap = !touchDidMove && elapsed < TAP_MAX_MS;

  if (wasTap) {
    // Tap on background — deselect hotspots if any selected
    if (selectedHotspot) {
      deselectAll();
    }
  } else {
    // Finger lifted mid-swipe — carry momentum forward
    velocity = lastDeltaX * ROTATE_SENSITIVITY;
...
// Desktop mouse: model-viewer handles this natively via camera-controls
// The locked phi/radius in min/max-camera-orbit ensures desktop also stays flat
