document.addEventListener('DOMContentLoaded', () => {
  ...
  function setupCameraControls() {
    if (isDesktop()) {
      // Desktop: remove camera-controls, implement pure pan-only
  ...
  function enterLockedMode() {
    if (!isDesktop()) {
      ...
      // Desktop: top-down view for focused navigation
      mv.removeAttribute('camera-controls');
      mv.setAttribute('interpolation-decay', '80');
  ...
  // ─────────────────────────────────────────
  // DESKTOP PAN-ONLY CAMERA (no orbit)
  // ─────────────────────────────────────────
  ...
  document.addEventListener('pointermove', (e) => {
    ...
    const panX = (deltaX * 0.8) / 100;
    const panZ = -(deltaY * 0.8) / 100;
  ...
  mv.addEventListener('wheel', (e) => {
    ...
    setTimeout(() => mv.setAttribute('interpolation-decay', '80'), 150);
  }, { passive: false });
