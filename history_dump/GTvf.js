document.addEventListener('DOMContentLoaded', () => {
  ...
  mv.addEventListener('pointermove', (e) => {
    ...
    const newPhi = Math.max(0.1, Math.min(Math.PI * 0.9, orbitStartPhi - (deltaY * ORBIT_SENSITIVITY)));
  ...
  // ─────────────────────────────────────────
  // DESKTOP SEARCH BAR (In Top Bar)
  // ─────────────────────────────────────────
  const desktopSearchInput = document.querySelector('.desktop-search-input');
  const desktopSearchPill = document.querySelector('.desktop-search-pill');
  ...
  if (desktopSearchInput) {
    desktopSearchInput.addEventListener('input', (e) => {
      ...
      hotspots.forEach(h => {
        ...
        const matchesSynonym = query && SEARCH_SYNONYMS[labelStr] && SEARCH_SYNONYMS[labelStr].some(syn => syn.includes(query) || query.includes(syn));
