document.addEventListener('DOMContentLoaded', () => {
  ...
  function setupCameraControls() {
    if (isDesktop()) {
      // On desktop: remove camera-controls so model-viewer doesn't intercept
      // We handle orbit/zoom ourselves via pointer + wheel events
  ...
  // ─────────────────────────────────────────
  // DESKTOP ORBIT + ZOOM (fully custom)
  // Left-click drag = orbit, scroll = zoom
  // ─────────────────────────────────────────
  let isOrbitDragging = false;
  let orbitStartX = 0, orbitStartY = 0;
  let orbitStartTheta = 0, orbitStartPhi = 0;
  const ORBIT_SENSITIVITY = 0.015; // Increased for Google Maps-like responsiveness
  // Track pointer ID to handle properly
  let orbitPointerId = null;

  // We attach directly to the document so model-viewer doesn't swallow events
  document.addEventListener('pointerdown', (e) => {
    if (!isDesktop()) return;
    if (e.button !== 0) return;

    // Don't start orbit if clicking on a hotspot, button, or UI element
    ...
    // Only orbit when clicking on the model-viewer canvas area
    const mvRect = mv.getBoundingClientRect();
    if (e.clientX < mvRect.left || e.clientX > mvRect.right ||
        e.clientY < mvRect.top  || e.clientY > mvRect.bottom) return;
    ...
    orbitStartTheta = orbit.theta;
    orbitStartPhi = orbit.phi;
  ...
  mv.addEventListener('pointermove', (e) => {
    if (!isDesktop() || !isOrbitDragging || e.pointerId !== orbitPointerId) return;

    const deltaX = e.clientX - orbitStartX;
    const deltaY = e.clientY - orbitStartY;

    // Orbit rotation (horizontal only - no tilt)
    const newTheta = orbitStartTheta - (deltaX * ORBIT_SENSITIVITY);
    const newPhi = orbitStartPhi; // Keep phi fixed - no tilt

    // Pan movement (camera target)
    const orbit = mv.getCameraOrbit();
    const currentRadius = orbit.radius;
    const target = mv.getCameraTarget();
    
    // Sensitivity for panning (adjust as needed)
    const PAN_SENSITIVITY = 0.1;
    const panX = deltaX * PAN_SENSITIVITY / 100;
    const panZ = -deltaY * PAN_SENSITIVITY / 100;
    
    const newTargetX = target.x + panX;
    const newTargetZ = target.z + panZ;

    mv.setAttribute('interpolation-decay', '1'); // instant while dragging
    mv.cameraOrbit = `${(newTheta * 180 / Math.PI).toFixed(2)}deg ${(newPhi * 180 / Math.PI).toFixed(2)}deg ${currentRadius.toFixed(2)}m`;
    mv.cameraTarget = `${newTargetX.toFixed(2)}m ${target.y.toFixed(2)}m ${newTargetZ.toFixed(2)}m`;
  ...
  document.addEventListener('pointerup', (e) => {
    if (!isDesktop()) return;
    if (!isOrbitDragging || e.pointerId !== orbitPointerId) {
    ...
    // Tap detection: if barely moved, treat as background click
    if (backgroundPointerSnapshot) {
      const moved = Math.hypot(
        e.clientX - backgroundPointerSnapshot.clientX,
        e.clientY - backgroundPointerSnapshot.clientY
      );
      if (moved < EMPTY_TAP_DEADZONE) {
