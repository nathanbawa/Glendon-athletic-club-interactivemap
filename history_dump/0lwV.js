document.addEventListener('DOMContentLoaded', () => {
  ...
  const SMOOTH_TRANSITION_DECAY = "80";
  ...
  const CAMERA_BOUNDS_Y_MIN = -4.0;
  const CAMERA_BOUNDS_Y_MAX = 1.0;
  ...
  function setupCameraControls() {
    if (isDesktop()) {
      // Desktop: enable camera-controls with pan enabled
  ...
  function enterLockedMode() {
    if (!isDesktop()) {
      ...
      // Desktop: same pan-enabled camera controls
      mv.setAttribute('camera-controls', '');
      mv.removeAttribute('disable-pan');
      mv.setAttribute('interpolation-decay', '80');
    }
  ...
  // Left-click: select hotspots (default model-viewer behavior)
  // Right-click: pan/navigate (default model-viewer with camera-controls)
  // ─────────────────────────────────────────
  ...
  const selectHotspot = (h) => {
    ...
    if (marker) {
      ...
      if (!hint) {
        ...
        // Desktop shows mouse prompt, mobile shows hand gesture
        const hintContent = isDesktop() 
          ? `<span class="hint-text">Left click to select</span>`
    ...
    if (pos) {
      ...
      if (parts.length === 3) {
        ...
        if (isDesktop()) {
          mv.setAttribute('interpolation-decay', SMOOTH_TRANSITION_DECAY);
        } else {
          mv.setAttribute('interpolation-decay', SMOOTH_TRANSITION_DECAY);
          applyQuickCameraTransition();
        }
  ...
  // Tap outside hotspot to recenter
  document.addEventListener('click', (e) => {
    if (!selectedHotspot) return;
    // Don't recenter if clicking on UI elements
    if (e.target.closest('button, input, a, #top-bar, #desktop-controls-left, #desktop-legend-container, #info-dropdown-container, #itinerary-btn, .mobile-bottom-right-controls, .Hotspot')) return;
    deselectAll();
  });
  ...
  if (legendBtn && legendMenu) {
    legendBtn.addEventListener('click', () => {
      legendMenu.classList.toggle('show');
    });
  }
  ...
  // ─────────────────────────────────────────
  // CAMERA-CHANGE: pan clamp with Y bounds
  // Keep model visible by limiting vertical movement
  // ─────────────────────────────────────────
  modelViewer.addEventListener('camera-change', (e) => {
    ...
    // Clamp Y to keep model visible
    if (ny > CAMERA_BOUNDS_Y_MAX) { ny = CAMERA_BOUNDS_Y_MAX; clamped = true; }
    if (ny < CAMERA_BOUNDS_Y_MIN) { ny = CAMERA_BOUNDS_Y_MIN; clamped = true; }

    // Clamp X and Z for both platforms
    ...
    if (clamped && e.detail.source === 'user-interaction') {
      modelViewer.cameraTarget = `${nx}m ${ny}m ${nz}m`;
    }
  });
  ...
  const canShowAmbientPrompts = () => {
    ...
    return !selectedHotspot && !isItineraryMode && !infoOpen;
  };
  ...
  const showLegendPrompt = () => {
    if (isDesktop()) { schedulePromptCycle(); return; } // Desktop doesn't show legend prompt after gesture
  ...
  const showGesturePrompt = () => {
    ...
    if (gesturePromptEl) {
      // Update prompt for platform
      if (isDesktop()) {
        // Show left-click/right-click instructions
        const promptContent = gesturePromptEl.querySelector('.prompt-content');
        if (promptContent) {
          promptContent.innerHTML = `
            <div class="desktop-controls-hint">
              <div class="control-hint"><strong>Left-click</strong> to select</div>
              <div class="control-hint"><strong>Right-click</strong> to navigate</div>
            </div>
          `;
        }
      }
  ...
  document.addEventListener('pointerdown', () => { hideGesturePrompt(); });
