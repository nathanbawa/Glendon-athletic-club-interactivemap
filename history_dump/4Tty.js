document.addEventListener('DOMContentLoaded', () => {
  ...
  // ─────────────────────────────────────────
  // LEFT-CLICK DRAG FOR CAMERA ORBIT CONTROL
  // ─────────────────────────────────────────
  let isOrbitDragging = false;
  let orbitStartX = 0;
  let orbitStartY = 0;
  let orbitStartState = null;
  const ORBIT_SENSITIVITY = 0.005;

  mv.addEventListener('pointerdown', (e) => {
    // Only handle left-click (button 0) for orbit drag
    if (e.button !== 0) {
      return;
    }
    ...
    orbitStartState = modelViewer.getCameraOrbit();
    e.preventDefault();
  }, true);

  mv.addEventListener('pointermove', (e) => {
    if (!isOrbitDragging || !orbitStartState) {
      return;
    }
    ...
    // Update orbit based on mouse movement
    const newTheta = orbitStartState.theta + (deltaX * ORBIT_SENSITIVITY);
    const newPhi = Math.max(0.1, Math.min(Math.PI * 0.9, orbitStartState.phi - (deltaY * ORBIT_SENSITIVITY)));
    const currentRadius = orbitStartState.radius;

    modelViewer.cameraOrbit = `${(newTheta * 180 / Math.PI).toFixed(2)}deg ${(newPhi * 180 / Math.PI).toFixed(2)}deg ${currentRadius.toFixed(2)}m`;
  ...
  mv.addEventListener('pointerup', (e) => {
    ...
    orbitStartState = null;

    // Check if this was a small movement for tap handling
    const totalDeltaX = e.clientX - backgroundPointerSnapshot?.clientX || 0;
    const totalDeltaY = e.clientY - backgroundPointerSnapshot?.clientY || 0;
    const totalMoved = Math.hypot(totalDeltaX, totalDeltaY);

    if (totalMoved < EMPTY_TAP_DEADZONE && !selectedHotspot && !isItineraryMode) {
      e.preventDefault();
      e.stopPropagation();
    }
  }, true);
  ...
  mv.addEventListener('wheel', (e) => {
    ...
    const zoomSensitivity = 0.95;
    let newRadius = currentOrbit.radius;

    if (e.deltaY < 0) {
      newRadius *= zoomSensitivity; // Zoom in
    } else {
      newRadius /= zoomSensitivity; // Zoom out
    }
    ...
    modelViewer.cameraOrbit = `${(currentOrbit.theta * 180 / Math.PI).toFixed(2)}deg ${(currentOrbit.phi * 180 / Math.PI).toFixed(2)}deg ${newRadius.toFixed(2)}m`;
  }, true);

  // ─────────────────────────────────────────
  // BACKGROUND CLICK → deselect + recenter (LEFT-CLICK ONLY)
  // ─────────────────────────────────────────
  mv.addEventListener('pointerdown', (e) => {
  ...
  if (desktopLegendBtn && desktopLegendMenu) {
    ...
    // Close desktop legend menu when clicking outside
