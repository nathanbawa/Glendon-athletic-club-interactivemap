document.addEventListener('DOMContentLoaded', () => {
  ...
  // ─────────────────────────────────────────
  // DESKTOP LEFT-CLICK REMAPPER (left → pan)
  // Remap left-click to behave like right-click (pan instead of orbit)
  // ─────────────────────────────────────────
  if (isDesktop()) {
    mv.addEventListener('mousedown', (e) => {
      if (e.button === 0) { // LEFT CLICK
        e.preventDefault();
        // Simulate right-click behavior (pan) for left-click
        const evt = new PointerEvent('pointerdown', {
          pointerId: 1,
          clientX: e.clientX,
          clientY: e.clientY,
          buttons: 2, // RIGHT CLICK (pan)
          bubbles: true,
          cancelable: true
        });
    ...
    mv.addEventListener('mousemove', (e) => {
      const evt = new PointerEvent('pointermove', {
        ...
        clientX: e.clientX,
        clientY: e.clientY,
        buttons: (e.buttons & 1) ? 2 : 0, // Convert left (1) to right (2)
        bubbles: true,
        cancelable: true
      });
      mv.dispatchEvent(evt);
  ...
  if (desktopSearchBtn && desktopSearchInput) {
    desktopSearchBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      desktopSearchInput.classList.toggle('hidden');
      if (!desktopSearchInput.classList.contains('hidden')) {
  ...
  if (desktopSearchInput) {
    desktopSearchInput.addEventListener('input', (e) => {
      ...
      hotspots.forEach(h => {
        ...
        // Direct match
        const matchesDirect = query === '' || labelStr.includes(query);
        
        // Synonym match
        const matchesSynonym = query && Object.entries(SEARCH_SYNONYMS).some(([name, aliases]) =>
          labelStr.includes(name) && aliases.some(alias => alias.includes(query) || query.includes(alias))
        );
        
        const isVisible = matchesDirect || matchesSynonym;
  ...
  // Lock Y (vertical) for horizontal-only panning
  // ─────────────────────────────────────────
  modelViewer.addEventListener('camera-change', (e) => {
    ...
    // Desktop: lock Y at -1.3m (horizontal panning only)
    if (isDesktop() && e.detail?.source === 'user-interaction') {
      if (Math.abs(ny - (-1.3)) > 0.01) { ny = -1.3; clamped = true; }
    }

    // Mobile: lock Y based on context, also clamp X/Z
    if (!isDesktop()) {
      if (Math.abs(ny - (-1.3)) > 0.01) { ny = -1.3; clamped = true; }
    }
