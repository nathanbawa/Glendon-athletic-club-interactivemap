document.addEventListener('DOMContentLoaded', () => {
  ...
  // ─────────────────────────────────────────
  // DESKTOP LEFT-CLICK DRAG = MOVE CAMERA
  // ─────────────────────────────────────────
  ...
  mv.addEventListener('pointermove', (e) => {
    if (!isDesktop() || !isDragging || e.pointerId !== dragPointerId) return;

    const deltaX = e.clientX - dragStartX;
    const deltaY = e.clientY - dragStartY;

    const target = mv.getCameraTarget();
    const panX = (deltaX * 0.4) / 100;
    const panZ = -(deltaY * 0.4) / 100;
  ...
  mv.addEventListener('mouseenter', () => {
    if (!isDragging) mv.style.cursor = 'grab';
  });
