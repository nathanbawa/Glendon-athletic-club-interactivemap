document.addEventListener('DOMContentLoaded', () => {
  ...
  // ─────────────────────────────────────────
  const START_ORBIT  = "180deg 90deg 100m";
  ...
  function enterLockedMode() {
    if (window.innerWidth <= 768) {
      ...
      mv.setAttribute('min-camera-orbit', '0deg 90deg 30m');
      mv.setAttribute('max-camera-orbit', '0deg 90deg 200m');
      ...
      mv.setAttribute('min-camera-orbit', '180deg 90deg 30m');
      mv.setAttribute('max-camera-orbit', '180deg 90deg 200m');
      mv.setAttribute('interpolation-decay', QUICK_TRANSITION_DECAY);
    }
    mv.autoRotate = false;
  }
  ...
  const selectHotspot = (h) => {
    ...
    if (pos) {
      ...
      if (parts.length === 3) {
        ...
        // Slight zoom in: from 100m → 65m (top view)
        if (window.innerWidth <= 768) {
          mv.cameraOrbit = "180deg 90deg 65m";
        } else {
          // Desktop: top-view zoom
          mv.cameraOrbit = "180deg 90deg 65m";
        }
      }
    }
  ...
  if (itineraryBtn) {
    itineraryBtn.addEventListener('click', (e) => {
      ...
      requestAnimationFrame(() => {
        mv.cameraOrbit = "180deg 90deg 120m";
