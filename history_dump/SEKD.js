...
class GymViewer {
  ...
  createHotspots() {
    ...
    console.log(`✓ ${this.hotspots.length} hotspots créés et ajoutés`);
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (modelViewer) {
    // Attendre que model-viewer soit défini
    modelViewer.addEventListener('load', () => {
      new GymViewer();
    });
    
    // Fallback si l'événement load n'est pas déclenché
    setTimeout(() => {
      if (!window.gymViewerInitialized) {
        new GymViewer();
        window.gymViewerInitialized = true;
      }
    }, 2000);
  }
});
...
if (document.readyState === 'loading') {
  ...
  setTimeout(() => {
    const modelViewer = document.getElementById('modelViewer');
    if (modelViewer && !window.gymViewerInitialized) {
      modelViewer.addEventListener('load', () => {
        if (!window.gymViewerInitialized) {
          new GymViewer();
          window.gymViewerInitialized = true;
